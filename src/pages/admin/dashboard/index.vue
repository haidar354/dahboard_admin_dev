...
const tenantGrowthChartOptions = computed(() => ({
  ...
  xaxis: {
    categories: tenantGrowth.value.map(d => d.month),
...
const tenantGrowthSeries = computed(() => [
  {
    name: 'Tenant Baru',
    data: tenantGrowth.value.map(d => d.newTenants),
  },
  {
    name: 'Churned',
    data: tenantGrowth.value.map(d => d.churnedTenants),
  },
])
...
const revenueChartOptions = computed(() => ({
  ...
  xaxis: {
    categories: revenueData.value.map(d => d.month),
    labels: { style: { fontSize: '12px' } },
  },
  yaxis: {
    labels: {
...
const subscriptionSeries = computed(() => [
  subscriptionStatus.value.active || 0,
  subscriptionStatus.value.trial || 0,
  subscriptionStatus.value.expired || 0,
  subscriptionStatus.value.cancelled || 0,
])
