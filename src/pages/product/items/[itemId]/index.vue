...
const { defaultUnit, initializeDetailData } = useItemDetail()
...
const quickInfoCards = computed(() => [
  ...
  },
  {
    icon: 'tabler-scale',
    color: 'success',
    value: selectedItemDetail.value?.itemUnits?.length || 0,
    label: 'Satuan',
  },
  {
    icon: 'tabler-package',
    color: 'info',
    value: selectedItemDetail.value?.modifierGroups?.length || 0,
    label: 'Total Ekstra',
  },
  {
    icon: 'tabler-chef-hat',
    color: 'warning',
    value: selectedItemDetail.value?.boms?.length || 0,
    label: 'Bahan Resep',
  },
])
...
<template>
  ...
  <VFadeTransition>
    <div
      v-if="!isLoadingFetchDetail && selectedItemDetail?.useSameConfig"
    ...
    >
      ...
      <VRow>
        <VCol cols="12">
          ...
          <!-- Modifiers Data -->
          <VCard
            v-if="selectedItemDetail?.modifierGroups?.length"
            class="mb-6"
            elevation="2"
          >
            <VCardItem>
            ...
            <VCardText>
              <VTable>
                ...
                <tbody>
                  <tr
                    v-for="modifier in selectedItemDetail?.modifierGroups || []"
          ...
          <!-- Recipe Data -->
          <VCard
            v-if="(selectedItemDetail?.boms?.length || 0) > 0"
            class="mb-6"
            elevation="2"
          >
            <VCardItem>
            ...
            <VCardText>
              <VTable>
                ...
                <tbody>
                  <tr
                    v-for="bom in selectedItemDetail.boms"
                    :key="bom.itemBomId"
                  >
                    <td>{{ bom?.itemS?.name }}</td>
                    <td>{{ Number(bom?.quantity) }}</td>
                    <td>{{ Number(bom?.materialItem?.defaultUnit?.name) }}</td>
                  </tr>
                </tbody>
              </VTable>
            </VCardText>
