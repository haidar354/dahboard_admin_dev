...
import { useConfirmDialog } from "./confirmDialogStore";
  ...
  actions: {
    ...
    async fetchDetail(menuCategoryId: string) {
      ...
      try {
        ...
        if (response.data) this.selectedMenuCategory = response.data;
    ...
    async onDeleteMenuCategory(menuCategory: MenuCategory) {
      useConfirmDialog()
        .openDialog(
          `Kategori Menu "${menuCategory.name}"`,
          "Tindakan ini akan menghapus kategori menu dan tidak dapat dibatalkan."
        )
        .then(async (isConfirmed) => {
          if (isConfirmed) await this.delete(menuCategory.menuCategoryId);
        });
    },
