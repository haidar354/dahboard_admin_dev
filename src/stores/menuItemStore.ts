  ...
  actions: {
    ...
    async fetchDetail(menuItemId: string) {
      ...
      try {
        ...
        if (response.data) this.selectedMenuItem = response.data;
      } catch (error) {
    ...
    async create(menuId: string) {
      ...
      try {
        ...
        if (error?.data?.errors) this.formErrors = error.data.errors;
        throw error;
      } finally {
    ...
    async onDeleteMenuItem(menuItem: MenuItem) {
      ...
      await openDialog(
        `Menu Item "${menuItem.itemId}"`,
        "Apakah Anda yakin ingin menghapus menu item ini? Tindakan ini tidak dapat dibatalkan.",
        async () => {
